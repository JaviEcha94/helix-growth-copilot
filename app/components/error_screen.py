...
def render_error_screen(on_retry) -> None:
    ...
    st.markdown("<div style='max-width:520px;margin:60px auto;text-align:center;'>", unsafe_allow_html=True)
    ...
    st.markdown(f"<p style='color:#94A3B8;font-size:14px;'>{T['errorMsg']}</p>", unsafe_allow_html=True)
    if error_detail:
